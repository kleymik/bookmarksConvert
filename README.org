...
If no rootfolderWriteAreaPath argument is given,  bmksConvert.py prints org-mode format output to stdout,
for example:
...
(.webloc, .kdelnk, .desktop, .url,.. etc.), parses out the url and invokes  the browser to display it.
